...
| Tab | 功能 |
|-----|------|
| 🏋️ **训练** | MNIST 模型训练：支持 MLP / Transformer 架构切换、超参数调节、恢复训练、学习率调度（fixed / cosine）、振荡抑制、GPU 加速；实时训练曲线 |
| 🔍 **推理** | MNIST 图片推理：Top-K 预测 + 置信度条形图 + 图片预览；内置 ✍️ **手写板**，鼠标书写数字即刻识别 |
| 🖼️ **图片查看** | 浏览 CSV 格式手写数字图片，前后翻页导航 |
| 📝 **GPT 训练** | GPT 语言模型训练：架构参数配置、位置编码选择（learned / sinusoidal / alibi）、学习率调度、振荡抑制；实时 loss 曲线 |
...
5. **可选功能**：
   ...
   - ✅ 振荡抑制 — 自动检测 loss 震荡并降低学习率（可调窗口大小和阈值）
...
│  └─────────────────────────────────────────────┘   │
│                                                    │
│  位置编码: [learned ▾]  (learned/sinusoidal/alibi)  │
│                                                    │
│  ┌─ 学习率调度 / 振荡抑制 ─────────────────────┐   │
│  │ 同 MNIST 训练 Tab                             │   │
│  └─────────────────────────────────────────────┘   │
│                                                    │
│  ☑ GPU 加速（下拉: None / Vulkan / CUDA）  ☑ 显示梯度统计                        │
│                                                    │
│  [▶ 开始训练]  [⏹ 停止]                            │
│                                                    │
│  ┌─ 📊 训练曲线 ───────────────────────────────┐   │
...
4. **选择位置编码** — `learned`（可学习）/ `sinusoidal`（正弦波）/ `alibi`（线性偏置）
5. **配置超参数** — 轮数、学习率、批大小、序列长度、优化器
