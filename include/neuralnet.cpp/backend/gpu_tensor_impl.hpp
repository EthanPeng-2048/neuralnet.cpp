...
// 此文件在 matrix.hpp 末尾自动包含，解决循环依赖问题。
// ─────────────────────────────────────────────────────────────────────────
#pragma once
...
inline Result<GpuTensor> GpuTensor::from_matrix(const Matrix& cpu_mat, GpuBackend& backend)
{
    ...
    auto buf_res = GpuBuffer::create_device_local(
        ...
        cpu_mat.size(), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
...
inline Result<GpuTensor> GpuTensor::create_empty(std::size_t rows, std::size_t cols, GpuBackend& backend)
{
    auto buf_res = GpuBuffer::create_device_local(
        backend.device().device(), backend.memory_pool(),
        rows * cols, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                    VK_BUFFER_USAGE_TRANSFER_SRC_BIT |
                    VK_BUFFER_USAGE_TRANSFER_DST_BIT);
    if (!buf_res) return std::unexpected(buf_res.error());
